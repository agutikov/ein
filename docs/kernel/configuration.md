...
| `warn-derived-naf` | `bool` | `false` | emits one `warn` event per rule whose `(absent …)` guard watches a rule-derived relation, once, after root's saturation. A *stratification* signal since S1.21.8, not a soundness one | no | contract |
