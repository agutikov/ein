...
| [`events.py`](../../../ein.py/src/ein/events.py) | the **oracle event protocol** (M1a S1a.0.2) — `--events FILE`, one JSON object per line narrating every compile miss, enqueue, firing, mirror, park/admit/retire, quiescence, alternative justification, hypothesis verdict, entering, no-good and writeback. Off by default and free when off (`events.ON` is a module-level bool every call site tests first). Schema: [`events.md`](events.md). It was built for the Rust port's T2 parity tier — "the two engines took the same steps", the tier that pinned the *algorithm* rather than its answer — which retired with the second engine at M1a S1a.10.3; the format did not |
