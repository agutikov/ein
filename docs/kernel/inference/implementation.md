...
KB ─▶ Engine::compile_all ─▶ Plan ─▶ Saturator::saturate ─▶ reasoning facts
 ...
 hypgen ─▶ apriori (layer N sets) ─▶ commitment::try_commitment_set ─▶ detect
   ...
   │       blind      else                                                │
   │                                  └▶ verdict.rs reads k → Solution / Ambiguity /
   │                                     Contradiction / Open (k = 0, owed)
...
| module | role |
|--------|------|
| [`engine.rs`](../../../ein.rs/crates/ein-infer/src/engine.rs) | the per-(rule, activator) **compile cache**: `compile_all` / `compile_for`, `check_layout`, `fired`, and the NAF dependency map. It does not drive anything — the two-phase loop is `saturator.rs`'s `step` below. *ein.py's `Engine` also carried a queue-less `step()` that the `Saturator` wrapped; the port kept the cache and dropped the second loop, and this row said otherwise until 2026-08-31 (M1e `CD-M1`)* |
...
| module | role |
|--------|------|
| [`hypgen.rs`](../../../ein.rs/crates/ein-infer/src/hypgen.rs) | candidate enumeration (type-blind, S1.7.23); the filter pipeline (negated-facts / already-exists / lookahead / seen); `score_hypothesis`; `HypGenStats`. **⤳** also `complete` / `open_hypotheses` / `is_solution_node`, which were `hypgen.rs`: solution-node tracking is three predicates over what hypgen would propose, and separating them from it was a file boundary, not a seam. `generate_blind` (M1d S1d.3.1) is the ladder with its two upper rungs skipped — **not a search entry point**, but the only way to ask *how many facts would the blind enumerator still propose here?* of a node the active rung called complete. Its caller must hand it a KB it is willing to see written to, because a lookahead kill stores `(not h)`; `--json-summary`'s `leftover` block (`EIN_LEFTOVER=1`) runs it on a discarded fork, which is what makes the measurement a read |
...
| [`verdict.rs`](../../../ein.rs/crates/ein-infer/src/verdict.rs) | `Solution` / `Ambiguity` / `Contradiction` / `Open`; verdict read from the model count `k`; `Answer::Aborted` for a run a budget cut; `goal_bindings`. **`Open`** (M1d S1d.2.6) is `k = 0` with a debt rather than a conflict, and is why `k` — *models* — parts company with `stats.solution_nodes` — *recorded nodes*. Scoped by `OwesReport::in_scope`: only a program that states an obligation can reach it |
