...
`(co-located Norwegian House-1)` in the FACT layer and then derives
...
frontier terminal and takes no alternatives, so re-deriving the
FACT-layer `(co-located Norwegian House-1)` above leaves its
provenance untouched — a clue that happens to be re-derivable is still
a clue.
...
- [`../02-data-model/02_store.md`](../02-data-model/02_store.md) —
  KB store with layer views, fork, provenance (primary + recorded
