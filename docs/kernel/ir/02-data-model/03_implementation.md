...
| [`terms.rs`](../../../../ein.rs/crates/ein-core/src/terms.rs) | `Terms` — the three intern tables (symbols, integers, facts) held together, because a `Value`'s tag says which one to read. `cmp_semantic` is the total order over arguments; `STRUCTURAL` / `RESERVED` / `PREDICATES` are here because the **loader** needs them as well as the engine — `ein-ir`'s `macros.rs` and `imports.rs` both call `is_reserved`. Not the lexer, which reads nothing from this crate but its counters and keeps its own eleven-word `LEXER_KEYWORDS` (M1e S1e.4.2, `SE-L2`: two sets, one name, and the name is now two) |
