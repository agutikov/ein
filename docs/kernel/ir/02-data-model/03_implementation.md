...
| [`prov.rs`](../../../../ein.rs/crates/ein-core/src/prov.rs) | `Prov` (4 kinds) + its constructors, and the globally interned provenance arena. *Which* records a KB has is a per-KB table in `kb.rs`; the policy that decides what may be recorded is `Kb::record_justification` |
