...
**Stage:** [S1a.10.1](README.md#s1a101--bank-what-only-the-oracle-proves) · **Phase:**
[P1a.10](README.md#p1a10--one-implementation) · **Written:** 2026-08-20
...
> **The instrument is gone.** [S1a.10.3](README.md#s1a103--the-corpus-without-a-second-engine)
...
> **Closed 2026-08-20 by [S1a.10.2](README.md#s1a102--port-the-python-test-suite).** All 42 are
...
| 3.7 | **`ein-parity`'s relaxation is load-bearing** — 8 + 10 unit tests, and `utils/mutant_ein.py` deleting one event from a shipping binary's log | the D3 cut still catches a dropped productive firing | **covered** | the unit tests need no oracle. `mutant_ein.py` needed the harness to be its differ, so [S1a.10.3](README.md#s1a103--the-corpus-without-a-second-engine) moved the control into `cargo test`: `ein-infer/tests/event_cut_control.rs` runs the script's same three mutations against a stream it produces in-process. The mutation was always applied to the *artefact*, so the two processes bought nothing |
| 3.8 | **the acceptance gate** — 21 tests, the three zebra2 task classes | the *answer*, not the agreement | **covered**, partly; the rest is [S1a.10.2](README.md#s1a102--port-the-python-test-suite)'s | `ein-infer/tests/acceptance.rs` (3) + `ein-render/tests/idea08_acceptance.rs` (5) |

---

## 4. What the removal must relocate

Not a disposition — a **defect list for [S1a.10.5](README.md#s1a105--the-removal)**.
...
> **Done in [S1a.10.2](README.md#s1a102--port-the-python-test-suite), 2026-08-20**, a stage early
...
([S1a.6.6](README.md#s1a66--the-differential-fuzzer)), and
...
[S1a.10.4](README.md#s1a104--utils-re-aimed-at-one-engine) T1a.10.4.2 — the generator and the minimiser are
...
4. **`--jobs` invariance**, when [P1a.7](README.md#p1a7--parallelism)
...
> **The frontend half landed in [S1a.10.2](README.md#s1a102--port-the-python-test-suite)** as
...
>
> **The engine half landed in [S1a.10.4](README.md#s1a104--utils-re-aimed-at-one-engine)**, and the list
...
>   [S1a.7.5](README.md#s1a75--the---jobs-contract) is where the row
>   lands.
...
> **Softened, not avoided, by [S1a.10.2](README.md#s1a102--port-the-python-test-suite).** The
...
- [`corpus/README.md`](../../../corpus/README.md) — the corpus, and the five
  ...
  [S1a.10.3](README.md#s1a103--the-corpus-without-a-second-engine), which also moved the
