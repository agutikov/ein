...
**Twenty-five scripts, all but one driving `ein.rs`** — bar one,
...
| | | needs |
|---|---|---|
| [`render_examples.sh`](render_examples.sh) | every `.ein` under `examples/` → per-rule DOT + the constraint-scope and commitment-lattice views, + SVG. The input set is `find examples -path examples/broken -prune -o -name '*.ein' -print \| wc -l`; how many DOTs and lattices come out is what the run prints | `ein`, Graphviz |
...
| | | |
|---|---|---|
| [`stdlib_census.py`](stdlib_census.py) | what the seven `std.*` modules declare against what the corpus actually activates: every `(rule …)` in `stdlib/*.ein` parsed for module / parameters / priority / guard shapes, then every corpus entry × every declared `solve` / `saturate` / `test` run under `--events`, `fire` counted by rule and split productive vs redundant. Two things it must get right — a file that declares its own `symmetric` is not exercising `std.algebra`'s, and `normal` elides redundant firings, so three rules read as zero there and fire at `verbose`. `--check` exits 1 while any rule is at zero, or while a declared run exits 0 and narrates nothing (M1e S1e.4.5 — the second half was printed and never returned, so a *partial* sweep whose survivors still covered all 77 rules was green). **Since M1e S1e.4.5 it runs nightly**, in `.github/workflows/nightly.yml`'s `deep-corpus` job, beside `stdlib_mutants.py --check`: the two are the corpus-wide claim and the sensitivity claim, and both were the shape their own documents call *"runs when somebody remembers it"*. **It is no longer the gate** — [S1c.1.5](../docs/history/m1c_external_validation/README.md#s1c15--in-the-gate) made that `ein-infer/tests/stdlib_coverage.rs`, in-process, 0.04 s, and scoped to `tests/stdlib/` rather than to the whole corpus, because a rule that fires only inside `examples/zebra.ein` has no test. What stays here is the measurement the gate is a yes/no of: firings per rule, productive vs redundant, the sole-activator table, and `-k` for one directory's contribution. The measurement: [`stdlib_census.md`](../docs/history/m1c_external_validation/stdlib_census.md) — **38 of 73 rules never fired** on 2026-08-23 with `examples/zebra.ein` the sole activator of 20 more, and **0 of 73** on the [re-take](../docs/history/m1c_external_validation/stdlib_census.md#11-the-re-take--2026-08-24-and-the-zero-set-is-empty) after S1c.1.4 landed `tests/stdlib/`. Re-taken 2026-09-01: 77 of 77, zero set empty, 217 entries, 38 s | `ein`, nightly |
...
| [`stdlib_mutants.py`](stdlib_mutants.py) | does `tests/stdlib/` actually **kill a defect** in the rule it tests? — M1e [S1e.3.6](../plans/m1e_review_processing/p1e.3_medium/s1e.3.6_tests.md)'s instrument, and the fifth census. `tests/README.md` recorded **50 of 51** from M1c to M1e with one named survivor, and it was the only census here taken **by hand** — so the score rots as rules are added, in the one direction nobody checks. This applies four mechanical mutation families to the *s-expressions* of every `(rule …)` in `stdlib/*.ein` — `drop-neq`, `drop-absent`, `swap-premise`, `swap-assert` — copies the tree, points `$EIN_STDLIB` at the copy, and asks whether `ein test tests/` exits non-zero. Two things it has to get right: a splice that reproduces its own source is **not a mutant** (`(?R ?a ?a)` swapped is itself, and counting that either way is a lie), and the survivor set is banked as a **set** in [`tests/mutation_survivors.txt`](../tests/mutation_survivors.txt) rather than as a score, because a number decays while the thing it guards grows — `--check` fails on a new survivor *and* on a banked one that has since been killed, since that is an improvement worth banking. It is **exhaustive where the hand sweep was one-per-family**, so the numbers are different measurements: 217 mutants, **157 killed** (2026-08-31), and **48 of the 60 survivors are in `slots.ein`** — the module with the most parameterised, direction-sensitive rules and the fewest programs per rule. Not the gate: `ein-infer/tests/stdlib_coverage.rs` asks the cheaper question (is the rule *activated*), this one asks whether the activation would notice | `ein`, 7 s |
...
- **The corpus runner.** `ein-conformance run --tier T0…T3` swept the corpus
  over two engines; the sweep is `ein-cli/tests/corpus_cli.rs` now — every
  declared cell as a process, inside `cargo test`, and the count is
  `wc -l ein.rs/crates/ein-cli/tests/golden/corpus_exits.txt`, one line per
  cell of the full selection
