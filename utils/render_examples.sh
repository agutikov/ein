...
# the example dir. The **PoC-style unified "everything-on-one-page"
# view** is generated by `ein-bot kb dot` (S1.2.4) — one unified DOT
# per example, rendered through `fdp` for the PoC aesthetic:
# `<out>/<example>/_unified.dot`.
#
...
#                else `python3 -m ein_bot.cli` with PYTHONPATH=ein.py/src).
#   FORMATS    — space-separated Graphviz output formats to rasterise
#                (e.g. "svg pdf"). Defaults to "svg"; `--no-svg` forces
#                dot-only.
#
...
echo "       ${total_imgs} per-form renders, ${total_unified} unified renders"
echo "       under ${OUT_DIR}"
