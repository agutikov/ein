...
| P1.21 | Review response — [REVIEW_M1-01](REVIEW_M1-01.md) claims-vs-guarantees (P0 `state_hash` identity, P0 `unconditional_facts` retirement + doc sync, P1 `absent` semantics doc, P1 `minimal_unsat_core` truth-in-labelling, P2 ATMS-vs-CDCL positioning, P2 closure/worlds seam) | ✅ **core executed 2026-08-17** (S1.21.7/8 parked) | [`p1.21_review_response/`](p1.21_review_response/) (created 2026-08-16; each review point = investigation-report task + improvement task) |
| P1.22 | Obsolete-syntax purge (`(type …)`/`(instance …)` invalid per user ruling; block-head echoes; classic `zebra.ein`) + **M1 plans closeout** — final task deletes `plans/m1_core_graph_reasoning/` after rehoming live backlog (P1.9 E-catalog, P1.7c Track B, P1.21 divergences, open Qs) | open | [`p1.22_obsolete_syntax_and_closeout/`](p1.22_obsolete_syntax_and_closeout/) (created 2026-08-16; terminal M1 phase, strictly after P1.21; strictly serial) |
...
P1.11 **shipped + CLOSED** the package + CLI rename surfaced
