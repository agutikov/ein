...
- **Q5** ✅ (fixtures and audit; the `Known gaps` rewrite is step 5's) —
  `lattice/02`'s solution set derived **by hand** against
...
2. **Rule** ✅ — on `lattice/02` the default `k=3` agrees and `-L`'s `k=0` does
   not, so **the OFF side is wrong there**; the general statement is Q-M1e.6's:
   `complete` is a *sound but incomplete* approximation of clause 3 with the
   lookahead on, and a strictly weaker one with it off. Neither is the
   definition — and step 3's fixture shows the sharper case where **both** are
   wrong, which `lattice/02` cannot show because its ON side happens to be
   right.
3. **Build the fixture the pair needs** ✅ — built 2026-08-28 as
   ...
   `(config …)` line and one `:expect` apart. Typed by construction (an
   `(hrule …)` generates exactly three candidates, so the blind rung never
   runs and no model can bind a type as an object), **both halves exhaust** in
   six enterings, and the derivation is four lines in the header. Measured:
   ON `Solution k=1`, OFF `Contradiction k=0`, hand-derived **k=2**.
