# D9 — The kernel page overclaims, and the probe scheduled here is what would refute it

**Touches:** [`docs/kernel/inference/solution_semantics.md`](../../../../docs/kernel/inference/solution_semantics.md)
§6, committed 2026-08-28 in `429777f`.
**Should be settled before T1e.1.1.2 runs**, because that task is what tests
the claim.
...
**Recommended: A.** Proposed replacement:
...
This is a **doc** change to a page committed on this branch; it does not touch
the engine and does not wait on anything. If it is taken, it goes in before
T1e.1.1.2 runs, so the probe tests a claim the page actually makes.
