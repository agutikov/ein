...
| [P1e.4](p1e.4_low/README.md) | Low — 21 findings | 8 | **shipped 2026-09-01** | the one-line class, batched by topic — each stage one commit, each finding fixed, refuted or accepted with a reason at the site. **Twelve of the seventeen live findings were larger, smaller or different than reported**, including a reachable process panic (`CO-L1`), a premise whose named enforcement did not enforce it (`ST-L1`), a fabricated transcript (`CD-L2`) and a recommendation refuted by measurement (`TE-L3`) |
...
| `TE-L1` | wall-clock-sensitive assertions inside the deterministic gate | [S1e.4.5](p1e.4_low/s1e.4.5_tests.md) | **fixed 2026-09-01 — and the finding leads with the safe one.** Measured: the 20 s ceiling has a ~1 300× margin and cannot flake; the 4× `cost_ms` band trips at **2.9×**. That one is asserted under `EIN_CORPUS_SLOW` (nightly, `--release`, the profile it was measured on) and *reported* per commit; the flag-threshold directions stay. A **third** site the finding calls non-existent is `einb/roundtrip.rs`. And the worst hazard is not an assertion: `-T` in a `runs` column would put a stopwatch in `corpus_exits.txt`, now refused by `manifest::no_declared_run_budgets_by_wall_clock` |
| `TE-L2` | hard-coded world anchors couple four crates' tests to two puzzle files | [S1e.4.5](p1e.4_low/s1e.4.5_tests.md) | the list is measured — **six** crates and 26 test files, not four ([S1e.1.6](p1e.1_open_questions/s1e.1.6_coverage_gaps.md) T1, handed over). **fixed 2026-09-01, and generated rather than written**: it was **28** two days later, both additions from this milestone's own commits, and whatever goes in `zebra2.ein` is copied into four generated files. The registry is [`examples/README.md` § What an edit to these two files fans out into](../../examples/README.md), its list a marked block `world_anchors::the_anchor_list_is_the_greps_own_answer` diffs, and both puzzles point at it. Ring 4 is the sentence it exists for: **three** `from_ein_py/` goldens, and an edit that changes the parse spends them |
| `TE-L3` | `run_tests.sh --tests-only` also skips the bench smoke, contradicting its header | [S1e.4.5](p1e.4_low/s1e.4.5_tests.md) | **fixed 2026-09-01 by amending the header — the finding's own recommendation is refuted.** Running the bench under the flag cannot buy the property it is argued for (still a subset by six steps), restores half a pair (clippy type-checks the bench and the flag skips it), and costs **5.3 s** of release codegen against the **1.6 s** the flag saves. The header meanwhile contradicted *itself* — *five* at `:5`, *six* at `:91` — and so did `gate_steps.rs`'s own message. `what_tests_only_skips_is_what_the_script_guards` holds the header to the set it parses now |
| `TE-L4` | `stdlib_census.py --check` is wired to no gate or workflow | [S1e.4.5](p1e.4_low/s1e.4.5_tests.md) | **fixed 2026-09-01 — two steps in the nightly that already existed**, and `stdlib_mutants.py --check` with it: the stage's premise *"only this one has a `--check` already written"* was falsified by S1e.3.6, one phase earlier in this milestone. `--check` also **could not fail** for the case it is most exposed to — a declared run exiting 0 and narrating nothing was printed and never returned. Re-taken: 77 of 77, 217 entries, 38 s, where three shipped sites said 180 and 37 s — and the warrant two of them cite for stating a count exactly named a **test that does not exist** |
...
| `CD-L1` | the five history-page banners omit `ein test` from the CLI enumeration | [S1e.4.6](p1e.4_low/s1e.4.6_code_doc_consistency.md) | **fixed 2026-09-01 — six sites, not five.** `README.md`'s crate table carried the same enumeration, written the same day and stale for the same reason. Every other sentence of the banner checks out. Marked with `<!-- api-history-banner -->` and held by `api_banner.rs`: the five regions are one string, and every `COMMAND ein <name>` in `golden/help_shape.txt` is named in it — the banner cites the surface's owner instead of becoming a sixth list |
| `CD-L2` | guide chapter 4's transcript does not match actual output | [S1e.4.6](p1e.4_low/s1e.4.6_code_doc_consistency.md) | **fixed 2026-09-01 — and it was fabricated, not drifted.** Two hunks (the bindings *and* a missing rule line), and `git show b4d5158` adds that text on 2026-06-17 while that day's engine already printed one binding per line under a rule. No engine in this repo's history printed it — so a pin was not enough and the block is **generated**, by `guide_transcripts.rs` from the command in its marker. Chapter 2's three blocks stay hand-maintained (they are *excerpts*, and a diff cannot express an elision), which `docs/guide/README.md` now says — replacing a sentence false of half the guide |
| `CD-L3` | `render_lattice`'s fallback comment states a wrong reason at the one site that triggers it | [S1e.4.6](p1e.4_low/s1e.4.6_code_doc_consistency.md) | **fixed 2026-09-01 — the sentence is false twice, and a *third* surface disagreed.** Every caller that reaches it sets `store_lattice = true`, and `LatticeProof` has no `kb_index` for any value of the flag to fill; `04_dot_rendering.md` said `full` *needs* the flag. The phase's **second** named re-bless (it expected one): **337 of 9 015 digests plus one text line, and not one line count changed**. One exported `FULL_VIEW_FALLBACK`, and two tests — one per crate — make *agrees with the help text* checkable rather than inspected |
| `DO-L1` | small internal defects in otherwise-normative pages | [S1e.4.7](p1e.4_low/s1e.4.7_documentation.md) | **fixed 2026-09-01 — three of four, and one was already done.** (b) fell to T1e.2.2.3 on 2026-08-30. (a) is larger: *nine* over a ten-row table, **and** the sentence under it claimed all ten were pinned by a test that covered **nine** — the tenth refusal had no case anywhere; it does now. (c) is four DOT blocks on three pages drawing a relation as `Rule`'s hexagon, and the renderer settles it **negatively** — no shipped view nodes a relation at all, and the normative legend calls it a dashed edge |
| `DO-L2` | frozen measurements presented as current | [S1e.4.7](p1e.4_low/s1e.4.7_documentation.md) | **fixed 2026-09-01, and the review's *mechanism* is refuted** — S1d.2.4's activator facts moved neither site. `examples/README.md` is 3–9 % stale (44.1 / 28.2 ms re-taken) so its digits keep their date and gain the re-take; its *12 rules* is a size and was 14, so it becomes the command. The guide's `≥23×, 3336+, does not finish` is **misattributed**, not stale: `ein.py`/PyPy numbers cut by a 90 s budget, on an engine that left the tree — today 54× and 3 557, and it finishes in 1.5 s. And `features.md`'s own footnote said 3 557 was the search *run to the end*; it is where the default depth cap stops it |
| `MA-L1` | `DEFAULT_PRIORITY`'s doc comment is arithmetically self-contradicting | [S1e.4.8](p1e.4_low/s1e.4.8_maintainability.md) | **fixed 2026-09-01**, and the review's sub-claim **refuted**: the 900 band *does* exist (three `hypothesis-contradiction` rules; only the stdlib stops at 500), the comment is a verbatim port of ein.py's already-wrong one, and `obligations::priority_of` is a second consumer nobody had written down. The value is not merely unpinned but **unobservable** — no program mixes a banded rule with an unbanded one — so the pin **sandwiches** it between 999 and 1001. M1f S1f.5.6's claim to this finding is corrected rather than left standing |
| `MA-L2` | a literal ~22-space run inside the non-exhausted `Contradiction` headline | [S1e.4.8](p1e.4_low/s1e.4.8_maintainability.md) | **fixed 2026-09-01, and it is *smaller* than reported** — `ein-cli` never calls `render_answer`, so it reaches no CLI output at all; it reaches 31 `trace[answer]` digests and the `ein-render` public API. The pinning gap is the finding's real content and is exactly as reported: **nothing asserted either `Contradiction` headline**. The re-bless was predicted and then measured — 31 of 9 015, one op, no line count changed. And the sibling sweep is **not** empty: a 14-space run in `saturator.rs`'s assertion message |
| `MA-L3` | `summary.rs`'s `write()` doc comment contradicts the JSON writer's tested behavior | [S1e.4.8](p1e.4_low/s1e.4.8_maintainability.md) | **accepted 2026-09-01 — and the question *is* settleable, from git rather than from the goldens.** `4c1a5b3^:_summary.py:208` passed `ensure_ascii=False`, so the review's *one of the two is wrong* is false: **both** are, differently, and only one was reported — `summary.rs` is right about ein.py and wrong about its own function; `json.rs`'s *no caller overrides it* is wrong about ein.py, where **two** did. The port implements one of them, so the same run emits `\u2014` in the summary and an em dash in `--events`. The escaping stays, both comments are corrected, and the premise is pinned |
| `MA-L4` | `sanity -y` re-saturates parents with a fresh memo, polluting the live event stream | [S1e.4.8](p1e.4_low/s1e.4.8_maintainability.md) | **split 2026-09-01: memo fixed, cause refuted, and a third thing found.** Sharing the memo is right and free (66.9 → 63.1 ms on `zebra2 -e -y`) but removes **zero** events — `compile` fires on an *engine* miss, never a memo miss, which `engine.rs` says at the site. What `-y` really adds is **four** kinds (`compile` `enqueue` `fire` `quiesce`), inherent to narrating the extra saturations, so it is documented where `-y` is. And the cost every document states — `k+1` — is ein.py's docstring's error: it is `1 + k + alive parents`, up to **2k+1** |
