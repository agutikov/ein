...
| [S1e.2.3](s1e.2.3_naf_refutation_diagnostic.md) † | Q-M1e.9's containment — a diagnostic for a refutation resting on an `absent` | — | 1 d | ✅ **done 2026-08-30.** One walk, two questions — `derived_naf_warnings` *is* the new function over an empty eligible set. Census: **9 rules over 7 entries** of 60 syntactic matches, and it **corrects D4**: `connex` is activated twice and exposed neither time, while `std.elim`'s membership scans are. Gated per the stage's own rule 3, since the corpus is not silent. Probe banked as a corpus fixture with an `:expect` meant to break; **0** solve shapes and 0 exit codes moved |
