...
| 2026-08-18 | ein.rs P1a.6 / S1a.6.9 — **measurement, not a ship** | *138.1 ms* | *539.9 ms* | *1.02 s* | the resumed fork saturator (`Saturator::resume`, `--features fork-delta`, off by default and dormant without `EIN_FORK_DELTA=1`). What it buys: fork firings 38 136 → **9 834** on `zebra2 -e` and 113 746 → **26 656** on `zebra -e`, fork compiles → **0**, `solve zebra.ein -e` 525.6 → **392.6 ms** — *the one unmet target, crossed*. What it preserves, over 1.08 M enterings of the whole corpus compared fact by fact: the verdict, `k`, the models, the entering count, each entering's `kind`, and **every alive fork's fixpoint**. What it moves: T2 by −62.5 % / −75.9 % at `verbose` **and −58.8 % / −74.2 % at `normal`**, `--trace` 561 → 240 steps — and, unpredicted, the **proof structure**: 90 002 facts get a different primary justification, because a resumed fork's inherited parked candidates carry root's tiebreakers and the NAF boundary admits one per round. `test_idea08_acceptance::test_zebra2_fires_walkthrough_rules` fails on it: `symmetric` fires only at root. [baseline.md §11](../p1a.6_performance/baseline.md#11-the-resumed-fork-saturator-measured) |
| — | ein.rs P1a.6 (optimised, `--jobs 1`) | — | — | — | target ≤ 0.2 s / ≤ 0.4 s / ≤ 5 s — **`zebra2 -e` met, `zebra -e` 1.35× short**; `zebra -e` is now **72.6 % match/bind** self time |
| — | ein.rs P1a.7 (`--jobs 8`) | — | — | — | — |
