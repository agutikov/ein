...
| — | ein.rs P1a.6 (optimised, `--jobs 1`) | — | — | — | target ≤ 0.2 s / ≤ 0.4 s / ≤ 5 s — **all four met**, with 13 % of headroom on the tightest; `zebra -e` is now **84.8 % match/bind** self time, 75.4 % of it in `unify` + `try_candidate` + `walk` |
| — | ein.rs P1a.7 (`--jobs 8`) | — | — | — | — |
