...
| — | ein.rs P1a.6 (optimised, `--jobs 1`) | — | — | — | target ≤ 0.2 s / ≤ 0.4 s / ≤ 5 s — **all four met**, with 80 % of headroom on the tightest and both `-e` cells at **~110× PyPy**; `zebra -e` is **39.1 % match/bind**, and what is left is the NAF boundary (**37.6 % cumulative**) and the per-entering snapshot (10.4 %) — the same two on the blind-mode cells, at 28.5 % and 11.9 % |
| — | ein.rs P1a.7 (`--jobs 8`) | — | — | — | — |
...
0.196 s). The delta is the claim; the absolute is the method.
