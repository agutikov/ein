...
| **`ProvArena`** — *missing from this table until 2026-08-22* | — | **the one with a real write rate.** 100 % of enterings push a record; `branching/06 -e` 39 per entering, `features/01 -e` **2 135 093 records / 205 MB**, and `ProvArena::get` returns a `&Prov` exactly as the fact store returns a `&Row`. It is also where "memory scales with jobs" happens: nothing reclaims a dead fork's records until the run ends. [T1a.7.1.7](../p1a.7_parallelism/s1a.7.1_sync_shared_state.md#task-t1a717--the-provenance-arena) |
