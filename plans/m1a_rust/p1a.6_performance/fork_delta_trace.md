...
has the same primary justification. What changes is the **step list**.

| | before | after |
|---|---:|---:|
| trace steps (the solution node's firings) | 561 | **240** |
| trace file, lines | 13 311 | **6 101** |
...
each conclusion is already present.

### after — the resumed saturator

```
## Step 1 — `co-located-negative`
...
## Step 6 — `co-located-negative`
> co-located: (not (smoke-loc Lucky_Strike House-2)) ⟹ (not (smoke-loc …)).
```

**Step 2 is the hypothesis's own first consequence** — *Juice is drunk in
...
- **Steps 4–6 are the closure ping-ponging.** `co-located` is symmetric in
  ...
  against 95.6 % before.
- **Step 1 is the root's own unpropagated delta.** A singleton death writes
...
- **unchanged**: the verdict, `k`, the models, the entering count, each
  entering's `kind`, every **alive** fork's fixpoint fact for fact, stdout and
  the `--dump-states` tree once the firing counts are normalised out;
- **changed**: the firing lists themselves (T2 and T3), and — the finding this
  ...
  valid derivations is recorded first. See
  [baseline.md §11](baseline.md#11-the-resumed-fork-saturator-measured) for
  the numbers and
  [Q-M1a.18](../open_questions.md#q-m1a18--may-a-fork-stop-re-narrating-the-roots-fixpoint)
  for what they mean for the decision.
