# S1f.10.1 — The exclusion relation, measured before it is used

**Phase:** [P1f.10](README.md) (The structure of the hypothesis set)
**Estimate:** 3 days
**Depends on:** [Q-M1e.6](../../m1e_review_processing/open_questions.md#q-m1e6--what-is-a-solution-and-what-is-a-model)
...
because two things are unknown and both change what S1f.10.3 is:
...
- Its answer is recorded in
  ...
  `plans/m1f_hypothesis_and_documentation/p1f.10_hypothesis_structure/exclusion_census.md`
...
- **A null result is a result.** If the excluding fraction is small on every
  entry that is not a bijection puzzle, the census says so and P1f.10's
  estimate is cut before S1f.10.3 is written.
- Nothing in the engine changes. The census drives the shipped binary.

## Tasks

### Task T1f.10.1.1 — The oracle, outside the engine first
...
### Task T1f.10.1.2 — The full-KB arm, and the disagreement column
...
### Task T1f.10.1.3 — Batch it, and price the whole corpus
...
### Task T1f.10.1.4 — What it would have bought
...
**not** the pair count — is what S1f.10.3 removes, and it is the phase's
headline.
...
([S1e.1.1](../../m1e_review_processing/p1e.1_open_questions/s1e.1.1_search_soundness_probes/README.md)'s Notes
counted three, and [AR-M1](../../m1e_review_processing/README.md#the-findings) is a finding about the
habit). This stage does **not** build it either; it uses `--json-summary` and
`--events` like its siblings, and P1e.3 [S1e.3.4](../../m1e_review_processing/p1e.3_medium/s1e.3.4_architecture.md)
