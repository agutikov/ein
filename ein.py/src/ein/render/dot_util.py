...
per-form IR renderer (:mod:`ein.ir.to_dot`) and the rule renderer
(:mod:`ein.render.rules`) share one copy and one source of truth
...
def fact_label(relation_name: str, args: tuple) -> str:
    ...
    by the derivation-slice (:mod:`ein.render.slice`) and the
    lattice DAG (:mod:`ein.render.lattice_dag`).
    """
    parts: list[str] = []
    for a in args:
