...
:mod:`ein.ir.types` (no ``kb`` / ``inference`` imports), keeping the
``kb → ir`` layering intact. The loader (:mod:`ein.kb.from_ir`)
...
class Macro:
    ...
    ``body`` is the template :class:`~ein.ir.types.IRNode` an invocation
    expands into.
    """
    name: str
    params: tuple[str, ...]
